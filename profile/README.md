...
기존에는 인건비 절감과 운영 효율성 강화를 위해 키오스크를 도입했지만, 현재는 "굳이 키오스크를 사용할 필요가 없다"는 의견이 많아지고 있는 상황입니다. 따라서 기존 키오스크를 전면 교체하지 않고 최소한의 하드웨어와 소프트웨어 업데이트만으로 배리어프리 기능을 제공할 수 있는 혁신적인 솔루션이 필요하게 되었습니다.  
...
- **접근성**: 여러 장애 유형(시각, 청각, 지체 등)의 소비자 및 노인 등 신체적 장벽을 넘어 누구나 쉽게 사용할 수 있는 서비스 제공
- **경제성**: 기존 기기를 활용해 추가 비용을 최소화하며 구축 가능, 모듈형 설계 & On-Device AI 기술을 적극 활용하여 하드웨어적 요소를 최소화하고 소프트웨어적으로 효율적인 기술 적용
- **편의성**: 직관적인 UI/UX, 음성 인식, 센서 및 모터 활용을 통한 사용자 경험 극대화
- **실시간성**: 외부 네트워크 의존 없이 디바이스 자체적인 추론을 통해 실시간으로 빠른 연산 수행을 위한 On-Device AI 기술 적용
- **효율성**: 키오스크와 연동되는 소상공인용 모바일 앱 제공을통해 소상공인의 운영 효율성 증진
